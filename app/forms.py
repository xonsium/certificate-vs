...
class CertificateForm(FlaskForm):
    ...
    verification_code = StringField(
        "Verification code (6-64 characters, leave blank to generate)",
    ...
    def validate_verification_code(self, field):
        ...
        if not raw:
            return
        if len(raw) < 6 or len(raw) > 64 or not re.match(r"^[A-Za-z0-9]+$", raw):
            raise ValidationError("Must be 6-64 alphanumeric characters.")
